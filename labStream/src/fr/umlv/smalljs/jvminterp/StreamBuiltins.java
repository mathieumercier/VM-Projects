...
class StreamBuiltins {
	...
	private static ClassWriter createBuiltinClassWriter() {
	...
	private static JSObject finishFunctionCreation(JSObject global, ClassWriter cv, String functionName,
			MethodType methodType) throws AssertionError {
		byte[] instrs = cv.toByteArray();
		dumpBytecode(instrs);
	
		FunctionClassLoader functionClassLoader = new FunctionClassLoader(new Dictionary(), global);
		...
		return JSObject.newFunction(functionName, mh);
	}
	...
	static JSObject createStreamForEachFunction(JSObject global) {
		...
		ClassWriter cv = createBuiltinClassWriter();

		String functionName = "stream_forEach";
		MethodType methodType = genericMethodType(1 + 1);
		MethodVisitor mv = cv.visitMethod(ACC_PUBLIC | ACC_STATIC, functionName, methodType.toMethodDescriptorString(),
				null, null);
		mv.visitCode();
		...
		return finishFunctionCreation(global, cv, functionName, methodType);
	}
	...
	static JSObject createStreamFilterHasnextFunction(JSObject global) {
		...
		mv.visitInvokeDynamicInsn("const_funcall",
				"(Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;", BSM_CONST_FUNCALL, "==");
		mv.visitInvokeDynamicInsn("truth", "(Ljava/lang/Object;)Z", BSM_TRUTH);
		Label l0 = new Label();
		mv.visitJumpInsn(IFEQ, l0);
		mv.visitVarInsn(ALOAD, lookupThis);
